...
@torch.no_grad()
def fewevaluate(model,data_loader, device):
    model.eval()
    ...
    cout1 = torch.zeros(914)
    allcout1 = np.array(cout1).astype(int)
    for step, data in enumerate(data_loader):
        images, labels = data
        output = model(images.to(device))
        pred = torch.max(output, dim=1)[1]
        preds2 = pred.cpu().numpy()
        label2 = labels.cpu().numpy()
        total_preds1 = np.array(preds2).astype(int)
        total_labels1 = np.array(label2).astype(int)
        sum_num += torch.eq(pred, labels.to(device)).sum()
        for p1, k1 in zip(total_preds1, total_labels1):
            if (p1 == k1):
                allcout1[p1] += 1
    ...
    #     few_num = 0
    #
    # for p1, k1 in zip(train_label, allcout1):
    #     if (p1 < 6):
    #         few_acc += k1
    #         few_num += p1
    return sum_num.item(),allcout1
...
def dualevaluate(model1,model2,data_loader, device):
    ...
    for step, data in enumerate(data_loader):
        ...
        #F1score
    #     pre_mask = torch.zeros(output1.size()).scatter_(1, pred1.cpu().view(-1, 1), 1.)
    #     predict_num += pre_mask.sum(0)
    #     tar_mask = torch.zeros(output1.size()).scatter_(1, labels.cpu().view(-1, 1), 1.)
